...
```{r}
dataTemp <- trainData[, predictVar]
preProc <- preProcess(dataTemp)
preProc
dataTempCS <- predict(preProc, dataTemp)
trainDataCS <- data.table(data.frame(classe = trainData[, "classe"], dataTempCS))
```

Apply the centering and scaling to the test dataset.
```{r}
dataTemp <- testData[, predictVar]
dataTempCS <- predict(preProc, dataTemp)
testDataCS <- data.table(data.frame(classe = testData[, "classe"], dataTempCS))
```

## Build the prediction model
We will now create a model to predict the classe outcome variable using a Random Forest on the remaining predictor variables.
...
testCaseCS <- predict(preProc, testCase[, predictVar])
